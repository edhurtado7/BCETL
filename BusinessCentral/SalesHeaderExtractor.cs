...
public sealed class SalesHeaderExtractor
{
    private const string EntityName = "SalesHeaders";
    private const int BatchSize = 1000;
    private readonly BusinessCentralClient _client;
    private readonly SalesHeaderRepository _repository;
    private readonly WatermarkRepository _watermarks;
    public SalesHeaderExtractor(BusinessCentralClient client, SalesHeaderRepository repository, WatermarkRepository watermarks) { _client=client; _repository=repository; _watermarks=watermarks; }
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        try
        {
            Console.WriteLine("============================================================"); Console.WriteLine("Currently executing SalesHeaders"); Console.WriteLine("============================================================");
            ...
            var batch=new List<BcSalesHeader>(BatchSize);
            await foreach(var row in _client.ReadSalesHeadersAsync(from,ct)) { batch.Add(row); if(batch.Count==BatchSize) await Flush(); }
            ...
            Console.WriteLine(); Console.WriteLine("SalesHeaders extraction completed successfully."); Console.WriteLine($"Total fetched: {fetched:N0}"); Console.WriteLine($"Total inserted: {inserted:N0}"); Console.WriteLine($"Total updated: {updated:N0}"); Console.WriteLine($"Last Stamp PT: {Pt(max)}"); Console.WriteLine($"Last Stamp UTC: {Utc(max)}");

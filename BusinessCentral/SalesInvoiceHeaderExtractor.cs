...
public sealed class SalesInvoiceHeaderExtractor
{
    private const string EntityName = "SalesInvoiceHeaders";
    private const int BatchSize = 1000;
    private readonly BusinessCentralClient _client;
    private readonly SalesInvoiceHeaderRepository _repository;
    private readonly WatermarkRepository _watermarks;

    public SalesInvoiceHeaderExtractor(BusinessCentralClient client,
        SalesInvoiceHeaderRepository repository, WatermarkRepository watermarks)
    {
        _client = client;
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            var batch = new List<BcSalesInvoiceHeader>(BatchSize);

            await foreach (BcSalesInvoiceHeader row in _client.ReadSalesInvoiceHeadersAsync(requestFrom, ct))
            {
                batch.Add(row);
            ...
            {
                BcSalesInvoiceHeader[] payload = batch.ToArray();
                batch.Clear();
    ...
    {
        Console.WriteLine("============================================================");
        Console.WriteLine("Currently executing SalesInvoiceHeaders");
        Console.WriteLine("============================================================");
    ...
    {
        Console.WriteLine();
        Console.WriteLine("SalesInvoiceHeaders extraction completed successfully.");
